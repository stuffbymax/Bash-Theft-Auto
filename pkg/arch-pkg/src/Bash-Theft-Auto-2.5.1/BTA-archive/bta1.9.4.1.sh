...
SAVE_DIR="saves" # Define the save directory as a variable
...
load_game() {
  if [[ -f "$SAVE_DIR/player_name.sav" && -f "$SAVE_DIR/location.sav" && -f "$SAVE_DIR/cash.sav" && -f "$SAVE_DIR/health.sav" && -f "$SAVE_DIR/guns.sav" && -f "$SAVE_DIR/items.sav" && -f "$SAVE_DIR/body_armor_equipped.sav" ]]; then
...
# Check for existing save and load or initialize game
if [[ -d "$SAVE_DIR" ]] && load_game; then
	: # Game loaded, no further action needed here
else
  Game_variables # Initialize new game
fi
