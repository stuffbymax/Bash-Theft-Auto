...
bank_balance=0
BANK_INTEREST_RATE=1      # 1% daily interest, compounds at day rollover
...
black_market() {
	...
	while true; do
		...
		echo " 5. Drill             (\$2000) - Crack bank vaults (consumed on use)"
		echo " 6. Hacker Device     (\$3500) - Better vault odds (consumed on use)"
		echo " 7. Leave"
		echo "================================"
		read -r -p "Choice: " c
		case "$c" in
			...
			5) if (( cash >= 2000 )); then cash=$(( cash - 2000 )); items+=("Drill")
					echo "One vault drill. Heavy, but it works."; play_sfx_mpg "item_buy"
				else echo "Not enough cash."; fi;;
			6) if (( cash >= 3500 )); then cash=$(( cash - 3500 )); items+=("Hacker")
					echo "A laptop rig for bypassing vault security."; play_sfx_mpg "item_buy"
				else echo "Not enough cash."; fi;;
			7) return;;
			*) echo "Invalid.";;
		esac
...
save_game() {
	...
	(
		...
		echo "bank_balance@@@$bank_balance"
...
Game_variables() {
	...
	loan_rate=15; loan_enforcer_warned=false; bank_balance=0; rented_safehouse=""; safehouse_rent_day=0
...
while true; do
	...
	echo "21. City Reputation | 22. Bank"
	echo "G.  Gang Management"
	echo "------------------------------------------------------------"
